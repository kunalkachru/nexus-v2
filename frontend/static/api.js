...
function synthesizeIncidentFromStatus(status) {
  ...
  const replicaSummary = {
    ...
    runtime_comparison_summary: `Scaffold-only mode ranked '${candidateFixes[0]?.action || "the leading mitigation"}' as the leading mitigation candidate. Runtime replay has not validated this mitigation yet.`,
    baseline_outcome_class: "reproduced",
    best_mitigation_action: candidateFixes[0]?.action || "",
    best_mitigation_outcome_class: "inferred_only",
    best_mitigation_status_code: null,
    best_mitigation_duration_ms: null,
    best_mitigation_summary: `${candidateFixes[0]?.action || "No mitigation selected"} is the leading scaffold-only mitigation candidate.`,
