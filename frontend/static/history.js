    ...
    .map(
      (item) => `
        ...
        >
          ...
          <td><a class="inline-link" href="${window.NexusNavigation?.withReturnTo(`incident?nexus_incident_id=${encodeURIComponent(item.incident_id)}&history_view=1`) || `incident?nexus_incident_id=${encodeURIComponent(item.incident_id)}&history_view=1`}">Open</a></td>
        </tr>
      `
    )
    .join("");
