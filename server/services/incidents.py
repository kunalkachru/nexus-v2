...
from server.services.investigation import InvestigationContextBuilder
...
from server.services.priority import normalize_priority_label, priority_rank, shift_priority_label
...
class IncidentService:
    def __init__(
        ...
        *,
        ...
        self._investigation = InvestigationContextBuilder(
            observability=observability,
            tenancy_service=self._tenancy_service,
            build_enterprise_overlay=self._build_enterprise_overlay,
            apply_persisted_replay_packet=self._apply_persisted_replay_packet,
            queue_source_channel=self._queue_source_channel,
            guardian_context=self._guardian_context,
            build_fresh_intake_truth=self._build_fresh_intake_truth,
        )
    ...
    async def get_incident_context_v1(
        ...
        *,
        ...
        return await self._investigation.build_incident_context_payload(
            incident=incident,
            tenant_id=tenant_id,
            ...
            outcome_summary = {
                ...
                "summary": (
                    ...
                    if executed
                        ...
                        else "Guardian blocked the remediation plan. Review the policy notes and constraints before proceeding."
                    )
                ),
    ...
    async def trigger_replica_replay(
        ...
        *,
        ...
        if status == "pack_unsupported":
            message = "No bounded REPLICA pack is available for this incident class. Runtime replay is not supported; proceeding with inferred-only evidence."
        elif status == "replay_available":
            message = "Bounded REPLICA pack is available but runtime replay was not executed. Proceeding with inferred-only evidence."
        elif status == "relay_unavailable":
            message = "The external runtime host is not currently available or reachable. Proceeding with inferred-only evidence."
    ...
    def _format_handoff_as_markdown(
        ...
        *,
        ...
        support_posture: str,
        reviewer_traceability: dict[str, object],
    ) -> str:
        evidence_backing = support_posture
        ...
        reviewer_lines = [
            f"- **Latest governance actor**: {reviewer_traceability.get('governance_actor_label', 'System')}",
            f"- **Latest replay actor**: {reviewer_traceability.get('replay_actor_label', 'Not recorded')}",
            f"- **Latest delivery actor**: {reviewer_traceability.get('delivery_actor_label', 'Not recorded')}",
            f"- **Audit entries considered**: {reviewer_traceability.get('audit_entry_count', 0)}",
        ]
...
**Confidence**: {int(float(diagnosis.get('confidence', 0)) * 100)}% (Evidence posture: {evidence_backing})
...
Support coverage:
- **Support posture**: {support_posture}
- **Replay posture**: {validation_result.replace('**Validation**: ', '').replace('⧖ **Status**: ', '')}
...
---
*Generated by NEXUS — {_utc_now_iso()}*
"""
...
- **Audit Trail**: {reviewer_traceability.get('audit_entry_count', 0)} relevant entries
...
## Reviewer Traceability
- Latest governance actor: {reviewer_traceability.get('governance_actor_label', 'System')}
- Latest delivery actor: {reviewer_traceability.get('delivery_actor_label', 'Not recorded')}
...
- Audit Trail Entries: {reviewer_traceability.get('audit_entry_count', 0)}
- Latest Governance Actor: {reviewer_traceability.get('governance_actor_label', 'System')}
...
**Reviewer Traceability**
- Governance: {reviewer_traceability.get('governance_actor_label', 'System')}
- Audit entries: {reviewer_traceability.get('audit_entry_count', 0)}
...
---
Full incident details: NEXUS | {incident.get('id', 'unknown')}
"""

    def _resolve_support_posture(
        self,
        *,
        replica_summary: dict[str, object],
        tenant_support: dict[str, object] | None = None,
    ) -> str:
        tenant_support = tenant_support or {}
        support_state = str(tenant_support.get("support_state") or "").strip().lower()
        if support_state in {"runtime-backed", "inference-first", "unsupported"}:
            return support_state
        if replica_summary.get("runtime_executed"):
            return "runtime-backed"
        return "inference-first"

    def _audit_actor_label(self, entry: dict[str, object] | None) -> str:
        if not isinstance(entry, dict):
            return "System"
        payload = entry.get("payload") if isinstance(entry.get("payload"), dict) else {}
        actor_user_id = entry.get("actor_user_id") or payload.get("user_id") or entry.get("tenant_id") or "System"
        roles = entry.get("actor_roles") or []
        role_text = f" ({', '.join(str(role) for role in roles if str(role).strip())})" if roles else ""
        return f"{actor_user_id}{role_text}"

    def _build_reviewer_traceability(self, nexus_incident_id: str) -> dict[str, object]:
        audit_logs = get_audit_logs(nexus_incident_id)

        def latest_matching(*needles: str) -> dict[str, object] | None:
        ...
        governance_entry = latest_matching("guardian", "governance")
        replay_entry = latest_matching("replica_replay", "replay")
        delivery_entry = latest_matching("handoff", "delivery")
        export_entry = latest_matching("proof_export", "handoff_export", "governance_packet")
        latest_entry = audit_logs[-1] if audit_logs else None

        timeline = []
        for entry in audit_logs[-6:]:
            payload = entry.get("payload") if isinstance(entry.get("payload"), dict) else {}
            timeline.append(
                {
                    "event": str(entry.get("event_type") or "audit"),
                    "timestamp": str(entry.get("timestamp") or ""),
                    "actor": self._audit_actor_label(entry),
                    "actor_roles": list(entry.get("actor_roles") or []),
                    "status": str(payload.get("status") or payload.get("current_stage") or "recorded"),
                }
            )

        return {
            "audit_entry_count": len(audit_logs),
            "latest_actor_label": self._audit_actor_label(latest_entry),
            "latest_event": str((latest_entry or {}).get("event_type") or "audit"),
            "governance_actor_label": self._audit_actor_label(governance_entry),
            "replay_actor_label": self._audit_actor_label(replay_entry),
            "delivery_actor_label": self._audit_actor_label(delivery_entry),
            "export_actor_label": self._audit_actor_label(export_entry),
            "timeline": timeline,
        }
    ...
    async def build_engineering_handoff(
        ...
        *,
        ...
        tenant_support = dict(context.get("tenant_support") or {})

        evidence_backing = self._resolve_support_posture(
            replica_summary=replica_summary,
            tenant_support=tenant_support,
        )
        similar_incidents = memory_hits.get("similar_incidents", [])[:2]
        reviewer_traceability = self._build_reviewer_traceability(nexus_incident_id)
        ...
        handoff_text = generator(
            ...
            support_posture=evidence_backing,
            reviewer_traceability=reviewer_traceability,
        )
    ...
    async def build_governance_packet(
        ...
        *,
        ...
        execution_outcome = context.get("execution_outcome")
        tenant_support = dict(context.get("tenant_support") or {})
        support_posture = self._resolve_support_posture(
            replica_summary=replica_summary,
            tenant_support=tenant_support,
        )
        reviewer_traceability = self._build_reviewer_traceability(nexus_incident_id)
        ...
        timeline = [
            {
                ...
                "evidence_posture": support_posture if support_posture != "unsupported" else "unsupported",
            },
            {
                ...
                "evidence_type": support_posture,
                "evidence_posture": support_posture,
            },
            {
                ...
                "evidence_posture": "runtime-backed" if replica_summary.get("runtime_executed") else support_posture,
            ...
            },
            {
                ...
                "actor": reviewer_traceability.get("governance_actor_label") or "GUARDIAN",
                "actor_roles": [],
        ...
        if execution_outcome:
            timeline.append({
                ...
                "actor": reviewer_traceability.get("governance_actor_label") or "operator",
                "actor_roles": [],
                "summary": execution_outcome.get("summary", "Action executed"),
                "status": execution_outcome.get("status", "completed"),
                "evidence_posture": "runtime-backed" if execution_outcome.get("runtime_backed") else support_posture,
            })
        ...
        return {
            ...
            "evidence_posture": {
                "root_cause_evidence": support_posture,
