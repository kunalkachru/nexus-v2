...
These 3 failures exist in the baseline (120 passed, 3 failed). Gate: `pytest tests/ -q` passes as long as the failed count does not increase beyond 3 and no new failures appear.
...
```bash
# Primary — baseline is 120 passed, 3 failed (pre-existing); must not get worse
pytest tests/ -q
...
- test results: X passed, 3 pre-existing failures unchanged
```
...
- Never mark "done" while any gate is failing (beyond the 3 pre-existing failures)
...
- Tests: `pytest tests/ -q` → 123 passed, 0 failed
