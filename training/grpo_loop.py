...
class GRPOTrainer:
    ...
    def train(self, num_episodes: int = 30) -> TrainingSummary:
        ...
        for episode_index in range(num_episodes):
            ...
            steps = extract_trajectory(episode, self.policies)[: self.max_steps_per_episode]
    ...
    def _update_policies(self, steps: list[TrainingStepRecord], advantage: float) -> None:
        per_agent_advantages = compute_group_advantages(steps, advantage)
        for step in steps:
            policy = self.policies[step.agent_name]
            optimizer = self.optimizers[step.agent_name]
            probability = policy.probability()
            agent_advantage = per_agent_advantages.get(step.agent_name, advantage)
            gradient = -agent_advantage * max(0.05, 1.0 - probability)
